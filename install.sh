...
CHECKPOINT_FILE="/tmp/edict_install_checkpoint"
LOG_FILE="$REPO_DIR/data/install.log"
...
banner() {
  ...
  echo -e "${BLUE}║       安装向导 (增强版)                  ║${NC}"
  echo -e "${BLUE}╚══════════════════════════════════════════╝${NC}"
  echo ""
}

log()   { echo -e "${GREEN}✅ $1${NC}"; mkdir -p "$(dirname "$LOG_FILE")" && echo "[$(date)] $1" >> "$LOG_FILE"; }
warn()  { echo -e "${YELLOW}⚠️  $1${NC}" && echo "[$(date)] WARN: $1" >> "$LOG_FILE"; }
error() { echo -e "${RED}❌ $1${NC}" && echo "[$(date)] ERROR: $1" >> "$LOG_FILE"; }
