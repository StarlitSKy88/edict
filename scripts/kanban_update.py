...
STATS_FILE = _BASE / 'data' / 'kanban_stats.json'
...
from utils import now_iso, retry, write_json  # noqa: E402
...
def validate_task_id(task_id: str) -> tuple[bool, str]:
    ...
    # 格式: JJC-YYYYMMDD-NNN
    pattern = r'^JJC-\d{8}-\d{3}$'
    if not re.match(pattern, task_id):
        return False, f"任务ID格式错误，应为 JJC-YYYYMMDD-NNN，例如: JJC-{datetime.date.today().strftime('%Y%m%d')}-001"
...
def validate_state_transition(from_state: str, to_state: str) -> tuple[bool, str]:
    ...
    if to_state not in allowed:
        return False, f"非法状态转换: {from_state} → {to_state}，允许: {allowed}"
    
    return True, ""


def update_stats(cmd: str, success: bool = True):
    """更新命令统计"""
    try:
        stats = atomic_json_read(STATS_FILE, {"commands": {}, "errors": 0, "last_update": None})
        
        if cmd not in stats["commands"]:
            stats["commands"][cmd] = {"success": 0, "failed": 0}
        
        if success:
            stats["commands"][cmd]["success"] += 1
        else:
            stats["commands"][cmd]["failed"] += 1
            stats["errors"] += 1
