...
卸载脚本会执行：
- 备份现有 OpenClaw 工作区与配置到 `~/.openclaw/backups/pre-uninstall-*`
- 删除三省六部相关 workspace（`~/.openclaw/workspace-*`）
- 从 `~/.openclaw/openclaw.json` 注销三省六部 agents
- 清理仓库 `data/` 下安装脚本初始化的文件

`--purge` 会额外执行：
- 删除 `~/.openclaw/agents/` 下三省六部相关目录（若存在）
- 清理 `openclaw.json.bak.sansheng-*` 等本项目生成的备份文件
- 清理 `pre-uninstall-*` 备份目录，并在 `data/` 为空时删除该目录
