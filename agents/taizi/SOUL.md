...
### 第三步：发给中书省
用 `sessions_spawn` 调用中书省（`agentId: "zhongshu"`）并传达整理后的需求：

> ⚠️ 严禁使用 `sessions_send`、`subagents steer`、`exec: openclaw agent ...` 作为替代。
...
当中书省/尚书省通过 `sessions_spawn` 子任务链回传结果时，太子必须：
1. 在飞书**原对话**中回复皇上完整结果
2. 更新看板：
```bash
