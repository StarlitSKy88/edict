...
```
飞书群 → OpenClaw 插件 → Gateway → Nick Agent
                    │
                    └── 太子 Agent 接收消息
```
...
```
Telegram Bot → OpenClaw → Gateway → Nick Agent
```
...
- [Nick 架构文档](docs/task-dispatch-architecture.md)
- [飞书插件安装](https://github.com/cft0808/nick#飞书机器人部署)

---

**总结**: Nick 依赖 OpenClaw 作为消息网关和 Agent 调度底座,Nick 在此基础上构建了三省六部的任务流转和可视化看板。两者是上层应用与底层平台的关系。
